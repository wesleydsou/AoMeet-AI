...
import { createPresignedUploadUrl, usesB2Storage } from "@/lib/services/b2-storage";
...
export async function POST(request: Request) {
  ...
  try {
    const { uploadUrl, storageKey } = await createPresignedUploadUrl(objectKey, contentType);

    return NextResponse.json({ uploadUrl, storageKey, contentType });
  } catch (error) {
    return NextResponse.json(
